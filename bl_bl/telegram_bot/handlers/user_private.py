...
Для перевода вам нужно отправить видео удволетворяющее следующим требованиям:
    1. Видео должено быть вертикальным или видеосообщением (кружочком).
    2. На видео должен находиться один человек на расстоянии 1-1.5 метра от экрана, расположенный по середине и смотрящий в экран.
    3. Все жесты должны быть отчетливо видны.

Ограничения на запросы можно узнать по комманде /limits. 
""")

# @user_private_router.message(Command("info"))
# async def info_handler(message: Message):
#     await message.answer("По всем вопросам писать на почту alex@karachun.com и мы накажем индусов плохо выполняющих свою работу!!!")
...
async def other_handler(message: Message):
    await message.answer("Я умею работать только с видео. Пришлите видео и тогда я смогу его перевести 🤗. Вы можете ознакомится с правилами пользования ботом по комманде /help")
